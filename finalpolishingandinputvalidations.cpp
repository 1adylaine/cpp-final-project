...
string productCode(string name, string brand){
	string code = "";
	string binary = "";	
...
void addProduct(){
	...
	cout << "Name      : "; cin.getline(cin, name[count]);
	cout << "Brand     : "; cin.getline(cin, brand[count]);
	cout << "Item      : "; cin.getline(cin, item[count]);
	cout << "Quantity  : "; cin >> quantity[count];
	cout << "Price     : "; cin >> price[count];
	
	code[count] = productCode(name[count], brand[count]);
...
void editProduct(){
	...
	if(choice == 1){
		cout << "Enter New Name: ";
		cin >> ws; getline(cin, modifiedDetail);
	...
	code[index] = productCode(name[index], brand[index]);
...
void displayProduct(){
	...
	for(int i = 0; i < count; i++){
		...
		for(int j = 0; j < displayedcount; j++){
			if(displayedItems[j] == currentItem) {
...
void searchProduct(){
	...
	cin >> ws; getline(cin, searchProduct);
