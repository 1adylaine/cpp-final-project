...
int productIndex(char code[][50], int count, char searchCode[]){
	for(int i = 0; i < count; i++){
		if(strcmpi(code[i], searchCode) == 0){
...
//display specific product for edit and search etc.
...
void addProduct(){
	...
	cout << "Price     : "; price[count] = doubleChecker();
	if(price[count] < 0){
		cout << "Invalid input!\n\n";
		return;
	}
	...
	for(int i = 0; i < count; i++){
		if(strcmpi(name[i], name[count]) == 0 &&
		   strcmpi(brand[i], brand[count]) == 0 &&
		   strcmpi(item[i], item[count]) == 0){
			checkDuplicate = i; //if is equal to exact same existing item it returns its index which is != -1 meaning it's a duplicate
			break;   	
		}
	}
	
	//if its a duplicate it will ask if you want to update the quantity or price instead or not at all
	if(checkDuplicate != -1){
		...
		if(choice == 1){
			...
			quantity[checkDuplicate] += additional; 
			cout << "Quantity added!\n\n"; 		
		}
		else if(choice == 2){
			cout<< "Enter New Price: ";
			double newPrice = doubleChecker();
			if(newPrice < 0){
				cout << "Invalid input!\n\n";
				return;
			}
	...
	cout << "Product has been added succesfully!\n\n";
	count++;
}

 	//edit product depemding on what you want to edit
void editProduct(){
...
//pad for char arrays for display
...
//pads for double for display of price with decimal
string padDouble(double num, int width){
    string s = "";
    int intPart = (int)num;
    int decPart = (int)((num - intPart) * 100 + 0.5);
...
void displayProduct(){
	...
	//dislay product arranged by type of product
	for(int i = 0; i < count; i++){
		char currentItem[50];
		strcpy(currentItem, item[i]);
		
		bool alreadyDisplayed = false;
		//checks if the product is already displayed
		for(int j = 0; j < displayedcount; j++){
			if(strcmpi(displayedItems[j], currentItem) == 0) {
				alreadyDisplayed = true;
				break;
			}
		}
		//display product based on item type if not the same item type, display different section for that diffferent item type
		if(!alreadyDisplayed) {
			cout << "\n                       [ ITEM: "<< currentItem << " ]\n";
			cout << "---------------------------------------------------------------------\n";
			...
			for(int a = 0; a < count; a++){
				if(strcmpi(item[a], currentItem) == 0){
					cout << "| " << pad(code[a], 12) 
     				<< " | " << pad(name[a], 18)
     				<< " | " << pad(brand[a], 9)
     				<< " | " << padInt(quantity[a], 5)
     				<< " | Php " << padDouble(price[a], 6) << "|\n";
				}
			}
...
//searcg product using the index function and the displayspecificproduct function
void searchProduct(){
...
void deleteProduct(){
	...
	//confirmation before you delete the product
	cout << "\n---------- PRODUCT TO DELETE ----------\n";
	displaySpecificProduct(index);
	char confirm;
...
void displayMenu(){
	cout << "=========================================\n";
	cout << "|    P O W E R   R A N G E R   1 - C    |\n";
	cout << "|             B O U T I Q U E           |\n";
...
int main(){
	...
	do {
		...
		//ask user if they want to display the menu again
		if(running){
			do {
			...
			if(answer == 'N' || answer == 'n') running = false; 
		}
		if(running){ system("cls"); } //donot clear if case 6
	}while(running);
